...
const GatewayDetailPage: NextPage<GatewayDetailProps> = (props: GatewayDetailProps) => {
    const [gateway, setGateway] = useState<Gateway | null>(props.gateway);
    const [measurements, setMeasurements] = useState<MeasurementData | null>(props.measurements);
    ...
    const deleteHandler = async () => {
        ...
        try {
            const response = await fetch(`https://meteopoint-be.vercel.app/api/gateway/${gateway._id}`, {
                method: "DELETE",
                headers: {
                    "Content-Type": "application/json",
                },
    ...
    const setNewEndTime = (newEndTime: Date) => {
        if (setNewEndTimeTimeout.current) clearTimeout(setNewEndTimeTimeout.current);
    ...
    const setNewStartTime = (newStartTime: Date) => {
        if (setNewStartTimeTimeout.current) clearTimeout(setNewStartTimeTimeout.current);
        // Debouncing user input
        ...
        <>
            ...
            <Container>
                ...
                <GraphControls>
                    ...
                    <Selector value={granularity} onChange={(e) => setGranularity(parseInt(e.target.value))}>
...
export async function getServerSideProps({ query }) {
    try {
        ...
        const gatewayResponse = await fetch(`https://meteopoint-be.vercel.app/api/gateway/${id}`);
