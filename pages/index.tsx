...
export default function Home({ gateways }: HomeProps) {
    ...
    useEffect(() => {
        const checkGatewayStatus = async () => {
            ...
            const updatedGateways: Gateway[] = gateways.map((gateway) => {
                if (gateway.latest && new Date(gateway.latest.timestamp) < eightMinutesAgo) {
            ...
            try {
                for (const gateway of updatedGateways) {
                    const response = await fetch(`https://meteopoint-be.vercel.app/api/gateway/${gateway._id}/status`, {
                        method: "PUT",
                        headers: {
    ...
    return (
        <Container>
            <Head>
                <title>Meteoport</title>
                <meta name="description" content="Lightweight IoT weather station data visualizer" />
...
export async function getServerSideProps() {
    try {
        ...
        const [gwData, latestData] = await Promise.all([gwRes.json(), latestRes.json()]);
